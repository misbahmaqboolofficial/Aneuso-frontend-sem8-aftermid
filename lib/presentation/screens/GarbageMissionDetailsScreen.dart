...
import 'admin/MissionAssignmentScreen.dart';
...
class _GarbageMissionDetailsScreenState extends State<GarbageMissionDetailsScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          Positioned(top: -100, right: -50, child: _blurCircle(300, palePink.withOpacity(0.4))),
          Positioned(bottom: -150, left: -50, child: _blurCircle(400, softLilac.withOpacity(0.2))),
  ...
  Widget _buildWOWAppBar(BuildContext context) {
    return SliverAppBar(
      ...
      flexibleSpace: FlexibleSpaceBar(
        centerTitle: true,
        title: const Text('MISSION DATA', style: TextStyle(fontWeight: FontWeight.w900, color: darkPurple, fontSize: 16, letterSpacing: 4)),
  ...
  Widget _buildWOWStepper(int statusId) {
    ...
    final steps = ['Report Approved', 'Driver Assigned', 'Driver En Route', 'Cleanup In Progress', 'Garbage Collected'];
  ...
  Widget _buildWOWInfoCard() {
    return _lightCard(
      child: Column(
        children: [
          _wowInfoRow(Icons.person_pin_rounded, 'OPERATIONAL DRIVER', currentReport.driverName ?? 'AWAITING DISPATCH', brightPurp),
          _wowDivider(),
          _wowInfoRow(Icons.bolt_rounded, 'URGENCY TIER', currentReport.urgencyName ?? 'STANDARD', softLilac),
          _wowDivider(),
          _wowInfoRow(Icons.inventory_2_rounded, 'PAYLOAD ESTIMATE', '${currentReport.estimatedVolume} KG', brightPurp),
          _wowDivider(),
          _wowInfoRow(Icons.map_rounded, 'GEO SECTOR', currentReport.address, darkPurple.withOpacity(0.6)),
  ...
  Widget _buildWOWLiveTracking() {
    return Container(
      ...
      decoration: BoxDecoration(color: brightPurp.withOpacity(0.05), borderRadius: BorderRadius.circular(24), border: Border.all(color: brightPurp.withOpacity(0.1))),
      child: Row(
        children: [
          const Icon(Icons.stream_rounded, color: brightPurp, size: 28),
          const SizedBox(width: 20),
          Expanded(child: Text('Live tracking established. Mission personnel executing collection sequences in the designated sector.', style: TextStyle(fontSize: 13, color: darkPurple.withOpacity(0.7), height: 1.5, fontWeight: FontWeight.w600))),
        ],
      ),
    );
  }
